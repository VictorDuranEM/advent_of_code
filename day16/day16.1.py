...
def main():
    ...
    for i in range(side_len):
        follow_beam(tiles, SOUTH, 0, i)
        energized_tiles = count_energized_tiles(tiles)
        if energized_tiles > highest:
            highest = energized_tiles
        tiles = copy.deepcopy(original_tiles)
            
    # Starting from bottom
    for i in range(side_len):
        follow_beam(tiles, NORTH, side_len - 1, i)
        energized_tiles = count_energized_tiles(tiles)
        if energized_tiles > highest:
            highest = energized_tiles
        tiles = copy.deepcopy(original_tiles)
    ...
    # Starting from right
    for i in range(side_len):
        follow_beam(tiles, WEST, i, side_len - 1)
        energized_tiles = count_energized_tiles(tiles)
        if energized_tiles > highest:
            highest = energized_tiles
        tiles = copy.deepcopy(original_tiles)
